...
def TROT(DATASET_NAME, source_user, target_user, vali_or_test, n_state, reg_e, reg_cl, reg_ta, target_bags,
    ...
    for index, a_act in enumerate(activities_required):
        ...
        with open(DATASET_NAME + '_' + a_act + '_' + str(source_user) + '_' + str(Num_Seconds) + '_' + str(
            ...
            Model_a_act_s = mixture.GaussianMixture(n_components=n_state, covariance_type=Cov_Type)
            Model_a_act_s.fit(a_act_source_bags)
        ...
        with open(DATASET_NAME + '_' + a_act + '_' + str(target_user) + '_' + str(Num_Seconds) + '_' + str(
            ...
            Model_a_act_t = mixture.GaussianMixture(n_components=n_state, covariance_type=Cov_Type)
            Model_a_act_t.fit(a_act_target_bags[start_index:end_index, :])
    ...
    pred, acc = tsot.fit_predict_for_GMM_MEAN(
...
for S_feats, S_labels, T_feats, T_labels, DATASET_NAME in S_T_file_path_pairs:
    ...
    results = {}
    times = {}
    file_pre = 'TROT_Ideal_GMM_' + DATASET_NAME + '_' + S_feats.split('_')[2] + "_" + T_feats.split('_')[2]
